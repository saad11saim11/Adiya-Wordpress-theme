...
if( class_exists( 'CSF' ) ) {
	...
	$breadcrumb 	= $adiya['author_breadcrumb'];
	$bsidebar 		= $adiya['author_sidebar'];
	$titles 		= $adiya['author_titles'];
	$column 		= $adiya['author_column'];
	$background 	= $adiya['author_background'] ? $adiya['author_background']['url'] : get_template_directory_uri(). "/assets/images/header/07_header.jpg";
	$padding 		= $adiya['author_breadcrumb_padding'];
}
else{
	$breadcrumb 	= true;
	$bsidebar 		= true;
...
<section class="blog-page py-100">
	<div class="container">
		...
		<div class="sec-title home-3">
			<div class="row">
				<?php if( $adiya['author_sheading'] || $adiya['author_stitle']  ):?>
				<div class="col-lg-5">
					<?php if($adiya['author_sheading'] ):?>
						<h2><?php echo esc_html($adiya['author_sheading']);?></h2>
					<?php 
						endif;
						if($adiya['author_stitle'] ):
					?>
						<h3><?php echo esc_html($adiya['author_stitle']);?></h3>
					<?php endif;?>
				</div>
				<?php 
					endif;
					if( $adiya['author_sdesc'] ):
				?>
				<div class="col-lg-5 d-flex align-items-center">
					<p class="sec-explain"><?php echo wp_kses( $adiya['author_sdesc'] , adiya_expanded_alowed_tags()); ?></p>
				</div>
				<?php endif;?>
			</div>
		</div>
