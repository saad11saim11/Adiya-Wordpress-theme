...
if( class_exists( 'CSF' ) ) {
	...
	$breadcrumb 	= $adiya['category_breadcrumb'];
	$bsidebar 		= $adiya['category_sidebar'];
	$titles 		= $adiya['category_titles'];
	$column 		= $adiya['category_column'];
	$background 	= $adiya['category_background'] ? $adiya['category_background']['url'] : get_template_directory_uri(). "/assets/images/header/07_header.jpg";
	$padding 		= $adiya['category_breadcrumb_padding'];
}
else{
	$breadcrumb 	= true;
	$bsidebar 		= true;
...
<section class="blog-page py-100">
	<div class="container">
		...
		<div class="sec-title home-3">
			<div class="row">
				<?php if( $adiya['category_sheading'] || $adiya['category_stitle']  ):?>
				<div class="col-lg-5">
					<?php if($adiya['category_sheading'] ):?>
						<h2><?php echo esc_html($adiya['category_sheading']);?></h2>
					<?php 
						endif;
						if($adiya['category_stitle'] ):
					?>
						<h3><?php echo esc_html($adiya['category_stitle']);?></h3>
					<?php endif;?>
				</div>
				<?php 
					endif;
					if( $adiya['category_sdesc'] ):
				?>
				<div class="col-lg-5 d-flex align-items-center">
					<p class="sec-explain"><?php echo wp_kses( $adiya['category_sdesc'] , adiya_expanded_alowed_tags()); ?></p>
				</div>
				<?php endif;?>
			</div>
		</div>
