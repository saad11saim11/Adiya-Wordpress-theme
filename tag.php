...
if( class_exists( 'CSF' ) ) {
	...
	$breadcrumb 	= $adiya['tag_breadcrumb'];
	$bsidebar 		= $adiya['tag_sidebar'];
	$titles 		= $adiya['tag_titles'];
	$column 		= $adiya['tag_column'];
	$background 	= $adiya['tag_background'] ? $adiya['tag_background']['url'] : get_template_directory_uri(). "/assets/images/header/07_header.jpg";
	$padding 		= $adiya['tag_breadcrumb_padding'];
}
else{
	$breadcrumb 	= true;
	$bsidebar 		= true;
...
<section class="blog-page py-100">
	<div class="container">
		...
		<div class="sec-title home-3">
			<div class="row">
				<?php if( $adiya['tag_sheading'] || $adiya['tag_stitle']  ):?>
				<div class="col-lg-5">
					<?php if($adiya['tag_sheading'] ):?>
						<h2><?php echo esc_html($adiya['tag_sheading']);?></h2>
					<?php 
						endif;
						if($adiya['tag_stitle'] ):
					?>
						<h3><?php echo esc_html($adiya['tag_stitle']);?></h3>
					<?php endif;?>
				</div>
				<?php 
					endif;
				...
				<div class="col-lg-5 d-flex align-items-center">
					<p class="sec-explain"><?php echo wp_kses( $adiya['tag_sdesc'] , adiya_expanded_alowed_tags()); ?></p>
				</div>
				<?php endif;?>
			</div>
		</div>
